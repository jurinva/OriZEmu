    ...
    0:                                                // N8VEM
     ...
     begin
       ...
       if ((R0 and SD_CLK)<>(PrevR0 and SD_CLK))and   // SD_CLK changing
          ((R0 and SD_PWR)<>0)and                     // ïèòàíèå íà êàðòó ïîäàíî
           SDEnabled() then
       begin                                          // clock pin state changing
         if (PrevR0 and SD_CLK)=0 then
         begin                                        // ôðîíò èìïóëüñà CLK
           inc(InpBitCount);
           if (spiState<>SPI_PWR_STATE) then          // êàðòà ïðîèíèöèàëèçèðîâàíà
           begin
             if ((R0 and SD_CS)<>0) then              // êàðòà âûáðàíà
             begin
               if (R0 and SD_DOUT)=0 then
                 spiByte:=spiByte and $FE             // çàùåëêèâàíèå "0" ïî ôðîíòó (SPI 0)
               else
                 spiByte:=spiByte or 1;               // çàùåëêèâàíèå "1" ïî ôðîíòó (SPI 0)
               if (spiState=SPI_IDLE_STATE) and
                  ((spiByte and $C0)=$40) then        // ïðèøëà êîìàíäà
               begin
                 UpdateSpi;
                 InpBitCount:=0;
               end;
             end
             else                                     // êàðòà íå âûáðàíà -> ïåðåäà÷à ñëåäóþùåìó â êîëüöå (ò.å. íà âûõîä)
             begin
               if (R0 and SD_DOUT)=0 then
                 SPDR:=SPDR and $FE
               else
         ...
         begin                                        // ñïàä èìïóëüñà CLK
           rl(SPDR);
           if (spiState<>SPI_PWR_STATE) and (InpBitCount=8) then
           begin
             InpBitCount:=0;                          // èäåì ïîáàéòíî
             if ((R0 and SD_CS)<>0) then UpdateSpi;
           end;
           rl(spiByte);                               // ñäâèã ïî ñïàäó (SPI 0)
         end;
       end;
     end; {if}
    1: if Value<>R0 then
...
begin
  ...
  case Scheme of
   0: Port0:=Value;                                     // N8VEM
   1: if ((R0 and SD_PWR)<>0) then                      // ïèòàíèå íà êàðòó ïîäàíî
      begin                                             // MSX - è ôðîíò è ñïàä CLK ôîðìèðóþòñÿ àâòîìàòè÷åñêè ïî ñèãíàëàì /WR, /RD (â ïðåäåëàõ îäíîé êîìàíäû CPU)
// ôðîíò
        R1:=Value;
        inc(InpBitCount);
        if (spiState<>SPI_PWR_STATE) then              // êàðòà ïðîèíèöèàëèçèðîâàíà
        begin
          ...
          begin
            if ((R1 and MSXSD_WR)=0) then
              spiByte:=spiByte and $FE             // çàùåëêèâàíèå "0"
            else
              spiByte:=spiByte or 1;               // çàùåëêèâàíèå "1"
            if (spiState=SPI_IDLE_STATE) and
               ((spiByte and $C0)=$40) then        // ïðèøëà êîìàíäà
              InpBitCount:=8;                           
          end
...
procedure TSDController.rl(var bb: byte);       // ñäâèã âëåâî ñ öèêëè÷åñêèì ïåðåíîñîì
begin
  if (bb and $80)=0 then
    bb:=(bb shl 1)
  else
